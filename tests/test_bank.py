...
def test_deposit_is_capped_by_the_chest():
    ...
    s._drop_on_chest([(p, p._base_inventory.index("Chainmail"))])  # exactly fills the chest
    assert guild.bank_items == ["Chainmail"] and p._base_inventory == ["Rope"]

    s._drop_on_chest([(p, p._base_inventory.index("Rope"))])       # no room left
...
def test_multi_select_stashes_several_items_in_one_move():
    ...
    random.seed(6)
    p = Unit("player")
    p._base_inventory = ["Rope", "Rope", "Torch"]
...
def test_multi_select_deposit_is_all_or_nothing():
    ...
    random.seed(7)
    p = Unit("player")
    p._base_inventory = ["Chainmail", "Rope"]        # 10.0 kg + 2.0 kg -- 12 kg total
    guild = Guild([p], bank_capacity=10)
    s = _screen(guild, [p])

    picks = [(p, 0), (p, 1)]
    s._drop_on_chest(picks)
    assert guild.bank_items == [] and p._base_inventory == ["Chainmail", "Rope"]
...
def test_locked_item_stays_selectable_but_distribute_load_leaves_it():
    ...
    assert a._base_inventory == ["Rope"] and b._base_inventory == []   # stayed put

    s._drop_on_member(b, [(a, 0)])                    # still movable by hand
    assert a._base_inventory == [] and b._base_inventory == ["Rope"]
